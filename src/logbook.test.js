...
describe("Logbook Additional Coverage", () => {
  it("handles empty completed and upcoming hikes gracefully", async () => {
    mockAxiosInstance.get.mockResolvedValue({
      data: { rows: [], pendingHikes: [] },
    });
    mockAxiosInstance.post.mockResolvedValue({ data: { userDatas: {} } });
    ...
    expect(screen.queryByText(/Test Hike/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/Upcoming Hike/i)).not.toBeInTheDocument();
  });

  it("handles axios GET errors gracefully", async () => {
  ...
  it("opens the edit modal but submits changes correctly", async () => {
    ...
    const hikeButton = await screen.findByText("Editable Hike");
    fireEvent.click(hikeButton);
